...
// Deconstructor
IO_handler::IO_handler(pendel_kinematics *kin, float Ts) : di1(10*Ts,Ts),di2(10*Ts,Ts),counter1(PA_6, PC_7),
                            counter2(PB_6, PB_7),u_enable(PB_9), mot_pwm(PB_15),mot_dir(PB_14)
{
    this->m_kin = kin;
    u2pwm.setup(0,VOLTAGE,0.01f,.99f,.01f,.99f);
    mot_pwm.period_us(100);
    mot_pwm.write(.01);
	
    uw1 = Enc_unwrap_scale(4096*4,16);
...
void IO_handler::write_voltage(float u_des)
{
        ...
        mot_dir = u_des < 0;   // negative sign leads to positive rev (motor is inverted)
}
