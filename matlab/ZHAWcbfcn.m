...
% Version R2021a with new Chirp for BuR
...
%         end
    ...
    case 'copyScope'
        ...
        set_param(blk,'DataFormat','StructureWithTime');
        %set_param(blk,'SaveToWorkspace','off');
    case 'copyToWS'
        set_param(blk,'LinkStatus','none');
        set_param(blk,'SaveFormat','StructureWithTime');
        set_param(blk,'Name','data');
    case 'SetupSLDRT'
...
%         end
    ...
    case 'SetEncoderReset'
        ...
        if strcmp(reset,'on')
            ...
            set_param([gcb,'/Encoder Input'],'IndexPulse','2');
        else
            %set_param(gcb,'MaskDisplay','disp(''Encoder\nInput\nIndex Reset Off'')');
    ...
    case 'BlockUpdate'
        ...
        switch type
            ...
            case 'ENC'
                ...
                reset = get_param(blk,'reset');
                if strcmp(reset,'on')
                    set_param(blk_ad,'IndexPulse','2')%reset
                    %set_param(gcb,'MaskDisplay','disp(''Encoder\nInput\nIndex Reset On'')');
                else
